...
d3.chart.brush = function() {
  ...
  function chart(container) {
    ...
    rects.attr({
      x: function(d) { return scale(d.start_t);},
      y: 0,
      width: 1,
      height: height
    ...
    brush.on("brushend", function() {
      ...
      var filtered = data.filter(function(d) {
        return (d.start_t > ext[0] && d.start_t < ext[1])
      })
      g.selectAll("rect.events")
      .style("stroke", "")
      
      g.selectAll("rect.events")
      .data(filtered, function(d) { return d.server })
      .style({
        stroke: "#fff"
      })
  ...
  chart.highlight = function(data) {
    ...
    rects.data(data, function(d) { return d.server })
    .style("stroke", "orange")
